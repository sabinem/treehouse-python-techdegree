...
class Ship:
    ...
    def get_direction(self):
        ...
        while True:
            userinput_direction = \
                input('Choose: [v]vertical or [h]orizontal \n> ')
            input_list, input_string, input_length = \
                check_input(userinput_direction)
            if input_length != 1:
    ...
    def get_location(self, board):
        ...
        The ship is then placed to the left or downwards and takes up
        gridpoints according to its length.
        """
        while True:
            userinput_point = input(
                "Give startcoordinates as column row to place {} {}: "
                "Example B2\n> "
    ...
    def check_sunk(self, attack_point):
        """
        This method checks whether the current attack point sunk the ship,
        which is the case if it was the last missing hit for the ship.
        """
