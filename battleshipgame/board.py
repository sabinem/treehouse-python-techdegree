...
def board_value_view(value, hide_ships=False):
    ...
    """
    if value in BOARD_HIDDEN and hide_ships == True:
        return BOARD_EMPTY
    else:
        return value
...
class Board:
    """
    This is the board class. It is implemented as an ordered dictionary on a grid of rows and columns.
    """
    def __init__(self):
        """
        The boards dictionary is initialized with the value of '0' on each grid point.
        """
        self.board_dict = OrderedDict({key: value for key in [p for p in GRID] for value in [BOARD_EMPTY]})

    def row_to_string(self, row_nr, hide_ships=False):
        """
        This function builds a string out of the values of the board dictionary for a given row.
        """
    ...
    def board_as_list_of_row_strings(self, hide_ships=False):
        """
        The boards dictionaries values are taken and transformed as a list of strings.
    ...
    def set_ship(self, ship, point):
        ...
        for idx in range(0, ship.length):
            ...
            if nextpoint in GRID and self.board_dict[nextpoint] == BOARD_EMPTY:
                ...
                raise OverflowError(
                    'Ship does not fit on the board that way! {} is not on board'
                    .format(nextpoint))
            else:
                raise ValueError(
