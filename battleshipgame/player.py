...
from .board import BOARD_HIT, BOARD_HIDDEN, BOARD_EMPTY, BOARD_UNHIT_CHOICES
from .board import Board
from .basic import clear_screen, check_gridpoint, check_input
from .ship import BattleShip, PatrolBoat, Cruiser, AircraftCarrier, Submarine


class Player:
    """
    The two players of the Game are instances of the player class. It keeps
    track of their ships on the board and their status.
    """
    def __init__(self, opponent=None):
        """
        This sets the name of the player and initializes his board and the player status.
        """
    ...
    def get_name(self, opponent=None):
        ...
        while True:
            ...
            input_list, input_string, input_length = check_input(userinput_name)
            if input_length == 0:
                print('Players name must not be blank!')
                continue
            try:
                if userinput_name == opponent.name:
                    print('Players must have different names. {} is taken already by the other player'
    ...
    def get_attack_point(self, opponent):
        ...
        while True:
            ...
            try:
                ...
                print(error)
                continue
            if opponent.board.board_dict[attack_point] not in BOARD_UNHIT_CHOICES:
    ...
    def handover_control(self):
        """
        After a player's move he is asked to hand over control to his opponent by hitting any key.
        """
        input('{}, press any key when you are ready to give control to the other player.'
              .format(self.name))

    def setup_ships(self):
        """
        The player's ships are set up here. Afterward the player's board is printed and he is asked
        to hand over to his opponent.
        """
        print('Hello {}, now it is time for you to set up your battleships!'.format(self))
        self.board.print()
        self.ships_afloat = [BattleShip(self, self.board),
    ...
    def perform_turn(self, opponent):
        """
        The player's turn is performed. He is shown his opponent's board with all that he has currently
        acchieved. He is also shown his own board with what the opponent acchieved.
        He is then promted to chose a point on the grid to attack.
        The result of the attack is evaluated and presented to him.
        If he wins, the game ends. Otherwise, he is asked to hand over to his opponent.
        """
        ...
        if opponent.defeated:
            ...
            print("That is what {} has guessed so far from you:".format(opponent))
            self.board.print()
            sys.exit()
        else:
            self.handover_control()
    ...
    def point_take_hit(self, attack_point, attacker):
        """
        This method takes the hit and determines whether it is a hit or a miss.
        """
